...
with open(f"./logs/power_usage_{data_time}.log", "a") as f:
    while True:
        ##################### Power Fetch #####################
        ...
        f.write(f"{timestamp} | CPU Usage Percentage: {psutil.cpu_percent()} % | CPU Clock Speed: {psutil.cpu_freq().current} GHz | CPU Core Counts: {psutil.cpu_count()} | CPU Temperature: {avg_temp} C | RAM Usage Percentage:  {ram_per} % | Disk Usage: {disk_per} | Disk Read Speed : {read_speed} | Disk Write Speed : {write_speed} | Power Watts: {output} \n")
